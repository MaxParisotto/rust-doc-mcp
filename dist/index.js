...
export class RustDocServer extends BaseServer {
    ...
    async searchManual(query) {
        try {
            ...
            const searchResults = Array.from(document.querySelectorAll('*'))
                .filter((element) => element.textContent?.toLowerCase().includes(query.toLowerCase()))
                .map((element) => element.textContent?.trim())
                .filter((text) => text !== undefined && text.length > 20); // Filter out very short matches
            return {
                content: [{
                        type: 'text',
    ...
    formatErrorSolutions(solutions) {
        if (solutions.length === 0)
            return 'No solutions found for this error.';
     ...
     */
    async analyzeCargoToml(projectPath) {
        ...
        try {
            ...
            const processDependencies = async (dependencies, section) => {
                if (!dependencies)
                    return;
                for (const [crate, versionInfo] of Object.entries(dependencies)) {
                    let version = typeof versionInfo === 'string' ? versionInfo : versionInfo.version;
            ...
            const output = Object.entries(dependencyInfo).map(([key, info]) => {
