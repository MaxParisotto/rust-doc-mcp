...
export class DocFetcher {
    ...
    private async fetchErrorPatterns(): Promise<void> {
        try {
    ...
    private extractSolutionFromIssue(body: string): { description: string; code?: string } | null {
    ...
    private async fetchIntegrationPatterns(): Promise<void> {
        try {
    ...
    private extractIntegrationPatterns(content: string): Array<{
        name: string;
        description: string;
        code: string;
    }> {
